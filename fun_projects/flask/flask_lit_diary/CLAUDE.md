...
- After each step: stop, run tests, suggest one commit message, and
...
- Tests must always be run using: ./venv/bin/python -m pytest
- If tests fail, analyze the exact error and propose a targeted fix before retrying.
